...
def synthetic_kuramoto_sivashinsky(tmpdir_factory):
    ...
    nt = 14
    nx = 128
    pde = PDEDataConfig(n_scalar_components=1, n_vector_components=0, trajlen=nt, n_spatial_dim=1)
    pde.nx = nx
    ...
    for mode in modes:
        ...
        h5f_u = dataset.create_dataset("u", (num_samples, nt, nx), dtype=float)
        ...
        v = dataset.create_dataset("dxv", (num_samples,), dtype=float)
...
def test_navier_stokes_dataloader(synthetic_navier_stokes):
    ...
    time_future = 1
    time_gap = 0
    dps = DATAPIPE_REGISTRY["KuramotoSivashinsky"]
    for mode in modes:
        if mode == "train":
            train_dp = dps[mode](
                pde=pde,
