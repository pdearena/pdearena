...
@pytest.mark.parametrize("name", [k for k in COND_MODEL_REGISTRY.keys() if '_1d_' not in k])
...
def test_named_1d_cond_models(name, param_conditioning):
    ...
    model = get_model(args, pdedata).to(device)
    input = torch.randn(8, 1, 3, 64, device=device)
    ...
    if param_conditioning == "scalar":
        z = torch.randn(8, device=device)
    elif param_conditioning == "scalar_2":
        z = torch.randn(8, 2, device=device)
    else:
        z = None

    output = model(input, t, z)
    assert output.shape == (8, 1, 3, 64)
