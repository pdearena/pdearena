...
def compute_normstats_maxwell(datapath):
    ...
    stats = {
        "d_field": {
            ...
            "std": torch.tensor(np.sqrt(d_field_st["sqsum"] / d_field_st["count"] - d_field_st["sum"]**2 / d_field_st["count"]**2)),
        },
        "h_field": {
            "mean": torch.tensor(h_field_st["sum"] / h_field_st["count"]),
            "std": torch.tensor(np.sqrt(h_field_st["sqsum"] / h_field_st["count"] - h_field_st["sum"]**2 / h_field_st["count"]**2)),
        }
    }            
    return stats
